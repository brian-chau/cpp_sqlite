...
7. Install `g++-12`, `gcc-12`, `build-essential`, `sqlite3`, and `libsqlite3-dev`.
    1. Restart WSL.
    2. Run `sudo apt update` and `sudo apt upgrade`
    3. Run `sudo apt install g++-12 gcc-12 build-essential sqlite3 libsqlite3-dev`

8. Install `fmt`, `openssl`, and `curl` from source.
    1. FMT
        ...
        * Run the following commands:
            ...
            ```
    2. OpenSSL
        * Download the latest OpenSSL tar.gz file from here: https://www.openssl.org/source/
        * Run the following commands:
            ```
            sudo apt update && sudo apt upgrade
            sudo apt install build-essential checkinstall zlib1g-dev -y
            sudo apt upgrade
            sudo tar -xzf openssl-<version_number>.tar.gz
            cd openssl-<version_number>
            sudo ./Configure
            sudo make
            sudo make test
            ...
            ```
    3. cURL
        * Download the latest curl tar.gz file from here: https://curl.se/download.html
        * Run the following commands to install it:
            ```
            sudo apt update && sudo apt upgrade
            sudo tar -xzf curl-<version_number>.tar.gz
            cd curl-<version_number>
            sudo ./configure --with-openssl --with-gnutls
